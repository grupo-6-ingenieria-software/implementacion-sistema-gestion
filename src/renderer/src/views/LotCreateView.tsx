...
export function LotCreateView({
  ...
  const [selectedProduct, setSelectedProduct] =
    useState<LotProductOption | null>(null);
  const [search, setSearch] = useState('');
  const [searchResults, setSearchResults] = useState<LotProductOption[]>([]);
  ...
  useEffect(() => {
    ...
    async function prepareForm(): Promise<void> {
      ...
      const response = await window.appApi.invoke<LotPrepareResponse>(
        'lote:preparar',
        {
          ean13: initialEan13,
          usuarioId,
  ...
  function selectProduct(product: LotProductOption): void {
