...
export function normalizeWorkerFormValues(payload: unknown): WorkerFormValues {
  if (!isObject(payload)) {
...
export function normalizeWorkerListFilters(payload: unknown): Required<
  Pick<WorkerListFilters, "estado" | "rol" | "search">
> & {
  usuarioId?: string;
} {
  if (!isObject(payload)) {
