...
export async function validateProductInSql(
  ...
  const [result] = await executor.all<{
    ean13Valid: number;
    ean13Unchanged: number;
    nameValid: number;
    categoryValid: number;
    costValid: number;
    saleValid: number;
    marginValid: number;
    stockValid: number;
  ...
  else if (!result.ean13Unchanged)
    errors.ean13 = "El codigo EAN-13 no se puede modificar al editar.";
  if (!result.nameValid)
    errors.nombre = "El nombre del producto es obligatorio.";
  if (!result.categoryValid)
    errors.categoriaId = "Seleccione una categoria valida.";
  if (!result.costValid)
    errors.precioCosto =
  ...
  else if (!result.marginValid)
    errors.precioVenta = "El precio venta debe ser mayor que el precio costo.";
