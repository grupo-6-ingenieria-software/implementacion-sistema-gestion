...
export async function loadStockAlerts(database: DashboardDb): Promise<StockAlert[]> {
  const products = await database.all<{ productId: number; productName: string; ean13: string; categoryId: number; minimumStock: number }>(sql`
  ...
  `);
  const categories = await database.all<{ categoryId: number; categoryName: string }>(sql`
    SELECT categoria_id AS categoryId, categoria_nombre AS categoryName FROM categoria
  `);
  const stocks = await database.all<{ productId: number; currentStock: number }>(sql`
    SELECT producto_id AS productId, SUM(lote_cantidad_actual) AS currentStock FROM lote GROUP BY producto_id
  `);
  const stockById = new Map(stocks.map(row => [row.productId, Number(row.currentStock)]));
  const categoryById = new Map(categories.map(row => [row.categoryId, row.categoryName]));
  return products.map(row => ({ productName: row.productName, ean13: row.ean13,
    categoryName: categoryById.get(row.categoryId) ?? '', minimumStock: Number(row.minimumStock),
    currentStock: stockById.get(row.productId) ?? 0,
  })).filter(row => row.currentStock <= row.minimumStock)
    .sort((a,b) => a.currentStock - b.currentStock || a.productName.localeCompare(b.productName));
}

export async function loadExpirationAlerts(database: DashboardDb, now = new Date()): Promise<ExpirationAlerts> {
  const { dateKey } = getDashboardDay(now);
  const lots = await database.all<{ lotId: string; productId: number; availableQuantity: number }>(sql`
  ...
  `);
  const dates = await database.all<{ lotId: string; expirationDate: string }>(sql`
  ...
  `);
  const products = await database.all<{ productId: number; productName: string; ean13: string }>(sql`
    SELECT producto_id AS productId, producto_nombre AS productName, producto_ean_13 AS ean13
  ...
  for (const lot of lots) {
    const product = byProduct.get(lot.productId); const expirationDate = byDate.get(lot.lotId);
    if (!product || !expirationDate) continue;
    const daysRemaining = differenceInCalendarDays(expirationDate, dateKey);
    alerts.push({ lotId: lot.lotId, productName: product.productName, ean13: product.ean13,
      availableQuantity: Number(lot.availableQuantity), expirationDate, daysRemaining });
  }
  alerts.sort((a,b) => a.daysRemaining - b.daysRemaining || a.productName.localeCompare(b.productName));
  return { expired: alerts.filter(row => row.daysRemaining < 0), expiringSoon: alerts.filter(row => row.daysRemaining >= 0) };
}
...
export async function loadDailySaleRows(database: DashboardDb, now = new Date()): Promise<SaleRow[]> {
  const { startUtc, endUtc } = getDashboardDay(now);
  const sales = await database.all<Omit<SaleRow, 'subtotal'> & { saleId: string }>(sql`
  ...
  const ids = sql.join(sales.map(row => sql`${row.saleId}`), sql`, `);
  const details = await database.all<{ saleId: string; quantity: number; priceId: string }>(sql`
  ...
  `);
  const prices = details.length ? await database.all<{ priceId: string; price: number }>(sql`
    SELECT historial_precio_producto_id AS priceId, historial_precio_venta AS price FROM historial_precio_producto
    WHERE historial_precio_producto_id IN (${sql.join(details.map(row => sql`${row.priceId}`), sql`, `)})
  `) : [];
  const byPrice = new Map(prices.map(row => [row.priceId, Number(row.price)]));
  const subtotals = new Map<string, number>();
  for (const detail of details) subtotals.set(detail.saleId,
    (subtotals.get(detail.saleId) ?? 0) + Number(detail.quantity) * (byPrice.get(detail.priceId) ?? 0));
  return sales.map(({saleId, ...sale}) => ({ ...sale, subtotal: subtotals.get(saleId) ?? 0 }));
}
...
export async function loadAttendanceSummary(database: DashboardDb, now = new Date()): Promise<AttendanceSummary> {
  const { startUtc, endUtc } = getDashboardDay(now);
  ...
  const pendingWorkers = workers.filter(row => !present.has(Number(row.workerId)))
    .map(row => ({ workerId: Number(row.workerId), fullName: row.fullName }));
  return { activeWorkers: workers.length, workersWithAttendance: workers.length - pendingWorkers.length,
    workersWithoutAttendance: pendingWorkers.length, pendingWorkers };
}
