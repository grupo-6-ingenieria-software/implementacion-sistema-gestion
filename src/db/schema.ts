...
export const usuario = sqliteTable(
  ...
  (t) => [
    check("usuario_id_length", sql`length(${t.usuarioId}) BETWEEN 3 AND 50`),
    check("usuario_rol_enum", sql`${t.usuarioRol} IN ('dueno','trabajador')`),
  ],
);
...
export const historialPrecioProducto = sqliteTable(
  "historial_precio_producto",
  {
    ...
    historialFechaHoraVigenciaDesde: text("historial_fecha_hora_vigencia_desde")
      .notNull()
      .default(nowDefault),
    historialFechaHoraVigenciaHasta: text(
...
export const lote = sqliteTable(
  "lote",
  {
    ...
    esLotePerecible: integer("es_lote_perecible", {
      mode: "boolean",
    }).notNull(),
...
export const lotePerecible = sqliteTable("lote_perecible", {
  ...
  lotePerecibleFechaVencimiento: text(
...
export const mermaLote = sqliteTable(
  "merma_lote",
  {
    ...
    mermaLoteCantidadDescontada: integer(
      "merma_lote_cantidad_descontada",
    ).notNull(),
  },
  (t) => [
    check("merma_lote_uuid", uuidCheck("merma_lote_id")),
    check("merma_lote_cantidad_min", sql`${t.mermaLoteCantidadDescontada} > 0`),
...
export const intentoLogin = sqliteTable(
  "intento_login",
  {
    ...
    intentoNombreUsuarioIngresado: text(
      "intento_nombre_usuario_ingresado",
    ).notNull(),
...
export const tasaLegal = sqliteTable(
  "tasa_legal",
  {
    ...
    tasaLegalFechaVigenciaDesde: text(
  ...
  (t) => [
    ...
    check("tasa_legal_valor_range", sql`${t.tasaLegalValor} BETWEEN 0 AND 100`),
    check(
...
export const ventaEfectivo = sqliteTable(
  "venta_efectivo",
  {
    ...
    ventaEfectivoMontoRecibido: integer(
...
export const detalleVenta = sqliteTable(
  "detalle_venta",
  {
    ...
    historialPrecioProductoId: text("historial_precio_producto_id")
      .notNull()
      .references(() => historialPrecioProducto.historialPrecioProductoId, {
        onDelete: "restrict",
      }),
  },
  (t) => [
...
export const ventaLote = sqliteTable(
  "venta_lote",
  {
    ...
    loteId: text("lote_id")
      .notNull()
      .references(() => lote.loteId, { onDelete: "restrict" }),
    ventaLoteCantidadConsumida: integer(
      "venta_lote_cantidad_consumida",
    ).notNull(),
  },
  (t) => [
    check("venta_lote_uuid", uuidCheck("venta_lote_id")),
    check("venta_lote_cantidad_min", sql`${t.ventaLoteCantidadConsumida} > 0`),
...
export const anulacionVenta = sqliteTable(
  ...
  (t) => [check("anulacion_venta_uuid", uuidCheck("anulacion_venta_id"))],
);
...
export const logErroresTecnicos = sqliteTable(
  "log_errores_tecnicos",
  {
    ...
    logErroresDescripcionTecnica: text(
      "log_errores_descripcion_tecnica",
    ).notNull(),
    usuarioId: text("usuario_id").references(() => usuario.usuarioId, {
      onDelete: "set null",
    }),
  ...
  (t) => [
    ...
    index("idx_log_errores_modulo").on(
      t.logErroresModulo,
      t.logErroresFechaHora,
    ),
  ],
);
