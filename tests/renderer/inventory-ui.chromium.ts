...
it("forwards invalid inventory forms through Chromium, IPC controllers and SQL", async () => {
  ...
  const lotController = createLotController({
    ...
    register: (payload) =>
      fixture.db.transaction((tx) =>
        registerLotWithExecutor(tx, schema, payload),
      ),
  });
  const wasteController = createWasteController({
    ...
    register: (payload) =>
      fixture.db.transaction((tx) =>
        registerWasteWithExecutor(tx, schema, payload),
      ),
  });
            ...
            request.on("end", () => {
              void (async () => {
                try {
                  ...
                  const result =
                    message.channel === "producto:listar"
                      ? {
                          ok: true,
                          data: {
                            products: [],
                            categories: [{ id: 1, nombre: "Lacteos" }],
                          },
                        }
                      : message.channel === "lote:proveedores"
                        ? await lotController.handle(message.payload, {
                            channel: message.channel,
                          })
                        : message.channel === "producto:registrar"
                          ? await productController.handle(message.payload, {
                              channel: message.channel,
                            })
                          : message.channel === "lote:registrar"
                            ...
                            : await wasteController.handle(message.payload, {
                                channel: message.channel,
                              });
  ...
  try {
    ...
    await page.goto(
      `${server.resolvedUrls!.local[0]}tests/renderer/inventory-harness.html`,
    );
    ...
    await product
      .getByText("El precio costo debe ser un entero mayor o igual a 0.")
      .waitFor();
    await product
      .getByText("El stock minimo debe ser un entero mayor o igual a 0.")
      .waitFor();
    ...
    await lot
      .getByText("La cantidad debe ser un entero mayor que 0.")
      .waitFor();
    await lot
      .getByText("El costo del lote debe ser un entero mayor que 0.")
      .waitFor();
      ...
      () =>
        (
          window as unknown as {
            calls: Array<{ channel: string; payload: Record<string, unknown> }>;
          }
        ).calls,
    );
    for (const channel of [
      ...
      assert.equal(calls.filter((call) => call.channel === channel).length, 1);
    const productPayload = calls.find(
      (call) => call.channel === "producto:registrar",
    )!.payload;
    assert.equal(
      Number.isNaN(productPayload.precioCosto),
      true,
      "la vista conserva el vacío como inválido; nunca lo vuelve 0",
    );
    const counts = await fixture.db.all<{
      products: number;
      lots: number;
      wastes: number;
    }>(sql`SELECT
    ...
    console.log(
      "PASS RF01/RF05/RF10 vistas Chromium → IPC → controlador → SQL real",
    );
  } finally {
    await browser.close();
    await server.close();
  ...
  async function seedInventory(): Promise<void> {
    await fixture.db.run(
      sql`INSERT INTO categoria (categoria_id, categoria_nombre, categoria_exige_vencimiento) VALUES (1, 'Lacteos', 1)`,
    );
    await fixture.db
      .run(sql`INSERT INTO producto (producto_id, producto_ean_13, producto_nombre, producto_precio_venta, producto_stock_minimo, producto_estado, producto_fecha_registro, categoria_id)
    VALUES (1, '7802920000015', 'Leche', 1000, 1, 'activo', '2026-01-01', 1)`);
    await fixture.db
      .run(sql`INSERT INTO proveedor (proveedor_id, proveedor_rut, proveedor_nombre_razon_social, proveedor_nombre_contacto, proveedor_telefono, proveedor_correo_electronico)
    VALUES (1, '76543210-K', 'Proveedor', 'Juan', '912345678', 'p@example.com')`);
    await fixture.db
      .run(sql`INSERT INTO lote (lote_id, lote_cantidad_inicial, lote_cantidad_actual, lote_precio_costo, lote_fecha_hora_ingreso, es_lote_perecible, es_lote_no_perecible, producto_id, proveedor_id)
    VALUES ('00000000-0000-4000-8000-000000000101', 5, 5, 700, '2026-01-01', 1, 0, 1, 1)`);
    await fixture.db.run(
      sql`INSERT INTO lote_perecible (lote_id, lote_perecible_fecha_vencimiento) VALUES ('00000000-0000-4000-8000-000000000101', '2027-01-01')`,
    );
  }
}, 30_000);
