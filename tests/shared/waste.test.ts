...
describe("waste helpers", () => {
  ...
  it("rejects non-integer quantities instead of truncating them", () => {
    const payload = normalizeWasteRegisterPayload({
      ean13: "7802920000015",
      cantidad: "4.9",
      motivo: "dano",
    });

    expect(validateWasteRegisterPayload(payload).cantidad).toBe(
