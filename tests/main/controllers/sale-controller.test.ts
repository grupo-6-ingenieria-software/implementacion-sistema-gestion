...
function dependencies(state: 'sin_registro' | 'abierta' | 'cerrada') {
  const inspectCash = vi.fn().mockResolvedValue(
    state === 'sin_registro'
      ? { status: 'sin_registro' }
      : state === 'abierta'
        ? { status: 'abierta', cierreCajaId: 'caja-1', openedAt: '2026-06-12T08:00:00.000Z' }
        : { status: 'cerrada', cierreCajaId: 'caja-1', openedAt: '2026-06-12T08:00:00.000Z', closedAt: '2026-06-12T20:00:00.000Z' },
  );
  return {
    inspectCash,
...
describe('saleController contracts', () => {
  it.each(['sin_registro', 'abierta', 'cerrada'] as const)(
    ...
    async (status) => {
      ...
      const response = await controller.handle({}, { channel: 'venta:verificar-caja' });
  ...
  it('gives a closed cash register priority over a malformed cart', async () => {
    ...
    expect(response).toMatchObject({ ok: false, error: { code: 'BUSINESS_RULE' } });
    expect(deps.register).not.toHaveBeenCalled();
  ...
  it('exposes the read-only cart validation result through its own channel', async () => {
    ...
    const response = await controller.handle(payload, { channel: 'venta:validar-carrito' });
